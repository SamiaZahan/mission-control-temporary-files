...
class PriceCalculator extends React.Component {
    ...
    CALC_RATES = {
        AIRBRINGR_TRAVELLER_FEE_USD_PER_KG : 0,
        AIRBRINGR_RRR_USD_PER_KG : 0,
        AIRGRINR_MINCRR_USD_PER_KG : 0,
        BACKPACK_US_SALES_TAX_PERCENT : 0,
        BACKPACK_SERVICE_FEE_PERCENT : 0,
        BACKPACK_TRAVELLER_FEE_PER_POUND : 0,
        SAVINGS_FROM_BACKPACK_PERCENT : 0,
        SHIPPING_FEE_PER_KG: 699,
    }
    ...
    handleBasePriceChange(price){
        ...
        let grossTotal = airbringrProductPrice + this.state.shippingFee + this.state.ukVat;
        let airbringrTotal =  grossTotal + this.state.shippingCharge ;
    ...
    handleVatChange(vat){
        //let totalVat = (this.state.productBasePrice * vat)/100;
        let totalVat = vat * this.EXCHANGE_RATES.USD_TO_BDT;
        let grossTotal = this.state.airbringrProductPriceBDT + totalVat + this.state.shippingFee;
        let airbringrTotal =  this.state.airbringrProductPriceBDT + totalVat + this.state.shippingFee + this.state.shippingCharge;
            this.setState({
                ukVat: totalVat,
                airbringrTotalBDT : airbringrTotal,
                grossTotal:grossTotal,
    ...
    handleShippingFee(fee){
        let shippingFee = Number(parseFloat(fee).toFixed(2)) * this.EXCHANGE_RATES.USD_TO_BDT
        let grossTotal = this.state.airbringrProductPriceBDT + this.state.ukVat + shippingFee;
        let airbringrTotal = Number(parseFloat(this.state.airbringrProductPriceBDT).toFixed(2)) +  this.state.shippingCharge + this.state.ukVat + shippingFee;
        this.setState({
            airbringrTotalBDT : airbringrTotal,
    ...
    handlePoundToKgChange(weight){
        this.state.productBaseWeightKg = weight * this.EXCHANGE_RATES.POUND_TO_KG;
    ...
    handleBaseWeightChange(weight){
        ...
        let shippingCharge = Number(parseFloat(maxWeight).toFixed(2)) * this.CALC_RATES.SHIPPING_FEE_PER_KG;
        let airbringrTotal = Number(parseFloat(this.state.airbringrProductPriceBDT).toFixed(2)) + shippingCharge + this.state.ukVat + this.state.shippingFee;
    ...
    render() {
        ...
        <div className="m-section__content">
            ...
            <div className="form-group m-form__group">
                ...
                <input type='hidden' className='form-control col-md-12' name="product_origin" defaultValue={this.state.jsonOrder.origin}  style={{fontSize : 14}} disabled="true" />
        ...
        <div className="m-section__content">
                ...
                <div className="form-group m-form__group" style={{height : 50}}>
                      <h5 style={{width: 180, justifyContent : "left"}}>Discount : </h5>
                      <input type='text' className='form-control col-md-5' name="price_before_discount" defaultValue={this.state.jsonOrder.price_before_discount} step="0.01" style={{fontSize : 14}}/>
                </div> 
                
                <div className="form-group m-form__group" style={{height : 50}}>
                    <h5 style={{width: 180, justifyContent : "left"}}>BASE Price (£) : </h5>
                    <input type='text' required className='form-control col-md-5' name="base_price_lb" defaultValue={this.state.jsonOrder.base_price_lb} step="0.01" onChange={e =>  this.handleBasePriceChange(e.target.value || 0) } style={{fontSize : 14}}/>
                </div>
            <div className="form-group m-form__group" style={{height : 50}}>
                <h5 style={{width: 180, justifyContent : "left"}}>UK Vat (£) : </h5>
                <input type='text' required className='form-control col-md-5' name="item_vat_lb" defaultValue={this.state.jsonOrder.item_vat_lb} step="0.01" onChange={e =>  this.handleVatChange(e.target.value || 0) } style={{fontSize : 14}}/>
            </div>

            <div className="form-group m-form__group" style={{height : 50}}>
            <h5 style={{width: 180, justifyContent : "left"}}>Shipping Fee (£) : </h5>
            <input type='text' required className='form-control col-md-5' name="shipping_fee_lb" defaultValue={this.state.jsonOrder.shipping_fee_lb} step="0.01" onChange={e =>  this.handleShippingFee(e.target.value || 0) } style={{fontSize : 14}}/>
            </div>
            ...
            <div className="form-group m-form__group" style={{height : 50}}>
                <h5 style={{width: 180, justifyContent : "left"}}>Item Weight (OZ) : </h5>
                <input type='text' className='form-control col-md-5' name="base_weight_pound" defaultValue={this.state.jsonOrder.product_weight_kg * this.EXCHANGE_RATES.KG_TO_OZ} step="0.01" onChange={e => this.handleOzToKgChange(e.target.value || 0)}  style={{fontSize : 14}}/>
            </div>
        ...
        <div className="form-group m-form__group" style={{height : 50}}>
        <input type='text' className='form-control col-md-12' name="base_length_inch" defaultValue={this.state.jsonOrder.product_length_in} step="0.01" onChange={e =>  this.handleLengthInchChange(e.target.value || 0) } style={{fontSize : 14}}/>
            </div>
            </div>
        ...
        <h5 style={{justifyContent : "left"}}>W</h5>
            ...
            <div className="col-md-2"  style={{padding : 0}}>
        <div className="form-group m-form__group" style={{height : 50}}>
        <input type='text' className='form-control col-md-12' name="base_width_inch" defaultValue={this.state.jsonOrder.product_width_in} step="0.01" onChange={e =>  this.handleWidthInchChange(e.target.value || 0) } style={{fontSize : 14}}/>
            </div>
            </div>
        ...
        <h5 style={{justifyContent : "left"}}>H</h5>
            ...
            <div className="col-md-2"  style={{padding : 0}}>
        <div className="form-group m-form__group" style={{height : 50}}>
        <input type='text' className='form-control col-md-12' name="base_height_inch" defaultValue={this.state.jsonOrder.product_height_in} step="0.01" onChange={e =>  this.handleHeightInchChange(e.target.value || 0) } style={{fontSize : 14}}/>
            </div>
            </div>
            </div>
            </div>

            <div className="form-group m-form__group" style={{height : 50, display:"none"}}>
                <h5 style={{width: 180, justifyContent : "left"}}>Shipping Charge :</h5>
                <input type='text' className='form-control col-md-5' name="shipping_charge_usd" defaultValue={Number(parseFloat(this.state.shippingCharge).toFixed(2))} step="0.01" onChange={e =>this.handleShippingWeightChange(e.target.value || 0)} style={{fontSize : 14}}/>
            </div>
            ...
            <h5 style={{ paddingBottom : 10, paddingTop : 30, marginBottom: 5, display : "none" }}> Savings from BP : {Number(parseFloat(this.state.backpackSavingsBDT).toFixed(2))}</h5>
            <h5 style={{  paddingBottom : 10, marginBottom: 5, display : "none" }}> BP Total (৳) : {Number(parseFloat(this.state.backpackTotalBDT).toFixed(2))}</h5>
            <div style={{ display : "none" }}>
        ...
        <h5 style={{ padding : 5, marginBottom: 5 }}> USD Exchange Rate : BDT {this.EXCHANGE_RATES.USD_TO_BDT.toFixed(2)}</h5>
            ...
            <tr>
            <th  style={{ borderTop : 0}}><h5> Vat : <span id="uk_vat" style={{ float : "right" }}>{Number(parseFloat(this.state.ukVat).toFixed(2))}</span></h5> </th>
            </tr>
            <tr>
            <th  style={{ borderTop : 0}}><h5> Shipping Fee : <span id="internal_shipping_fee" style={{ float : "right" }}>{Number(parseFloat(this.state.shippingFee).toFixed(2))}</span></h5> </th>
            </tr>
            <tr>
            <th  style={{ borderTop : 0}}><h5> Gross Total :<span id="gross_total" style={{ float : "right" }}>{Number(parseFloat(this.state.grossTotal).toFixed(2))}</span></h5> </th>
            </tr>
            <tr>
            <th style={{ borderTop : 0}}><h5> Shipping Charge : <span id="shipping_charge" style={{ float : "right" }}>{Number(parseFloat(this.state.shippingCharge).toFixed(2))}</span></h5>
            ...
            <div>
            <h5 style={{ padding : 10, marginTop : 40, marginBottom: 5, color: "#fff", backgroundColor:"#005500", display:"none" }}> AF (%) :  {Number(parseFloat(this.state.airbringrServiceFeePercent).toFixed(2))}</h5>
        ...
        <h5 style={{ padding : 10, marginBottom: 5, display: "none" }}> RRR : ($) {this.CALC_RATES.AIRBRINGR_RRR_USD_PER_KG.toFixed(2)}</h5>
            ...
            <h5 style={{ padding : 10, marginBottom: 5, color: "#fff", backgroundColor:"#ff0101", display:"none"  }}> CRR (%) :  <span id="rct_crr_reached">{Number(parseFloat(this.state.crrReached).toFixed(2))}</span></h5>
            ...
            <h5 style={{ padding : 10, marginBottom: 5, display: "none" }}> CRR ($) :  <span id="rct_crr_usd">{Number(parseFloat(this.state.crrUSD).toFixed(2))}</span></h5>
            <h5 style={{ padding : 10, marginBottom: 5, display: "none"  }}> TF (%) :  {Number(parseFloat(this.state.airbringrTravelerFeePercent).toFixed(2))}</h5>
            </div>
            </div>
